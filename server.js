...
function Ping(index) {
  ...
  //
  tcpp.probe(servers[index].ip, servers[index].port, function (err, available) {
    ...
    if (available) {
      ...
      if (!servers[index].online) {
        UpdateIncident.Update(servers[index].incidentid, 4, "It looks like the issues have been resolved! We will continue to monitor the service!");
      ...
      if (servers[index].online) {
        var incident = {
          ...
          message: "We're investigating connectivity issues with the " +
            servers[index].name +
            " server.",
